...
class InstagramManager:
    ...
    def upload_video(self, video_path, custom_caption=None):
        """Professional video upload with retry mechanism"""
        ...
        try:
            ...
            if not dashboard:
                dashboard = {
                    ...
                    'avg_engagement': 0,
                    'avg_viral_score': 0,
                    'best_post': {'filename': 'No posts yet', 'engagement': 0, 'viral_score': 0}
                }
            ...
            return {
                ...
                'avg_viral_score': 0,
                'best_post': {'filename': 'No posts yet', 'engagement': 0, 'viral_score': 0},
                'shadow_ban': {'is_shadow_banned': False, 'drop_percentage': 0},
...
⏭️ Skipped: {results['skipped']}
    ...
    def sync_existing_posts(self):
        """Sync all existing Instagram posts to database"""
        if not self.client or not self.current_username:
            return False, "Not logged in!"
        
        try:
            print("🔄 Syncing existing Instagram posts...")
            
            user_info = self.client.user_info_by_username(self.current_username)
            medias = self.client.user_medias(user_info.pk, amount=50)
            ...
            for media in medias:
                ...
                try:
                    ...
                    synced_count += 1
                    print(f"✅ Synced: {media_id} - {likes} likes, {comments} comments")
                    
                    time.sleep(2)
                    
                except Exception as e:
                    print(f"⚠️ Could not sync {media_id}: {e}")
                    continue
            
            print(f"✅ Sync complete! {synced_count} posts synced.")
            return True, f"Synced {synced_count} posts from Instagram"
